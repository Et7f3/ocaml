...
let transl_class ids cl_id pub_meths cl vflag =
  ...
  and linh_envs =
    List.map
      (fun (_, p) -> Lprim(Pfield (3, Fld_tuple), [transl_normal_path p], Location.none))
      (List.rev inh_init)
  in
  let make_envs lam =
  ...
  in
  let inh_keys =
    List.map (fun (_,p) -> Lprim(Pfield (1, Fld_tuple), [transl_normal_path p],
                                 Location.none))
      inh_paths
  in
  let lclass lam =
  ...
  and lclass_virt () =
    lset cached (Lfunction{kind = Curried; attr = default_function_attribute;
                             loc = Location.none;
